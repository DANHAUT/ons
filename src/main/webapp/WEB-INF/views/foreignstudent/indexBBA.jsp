...
<div id="availableList" style="margin-bottom:10px;"><!-- debut de Ligne -->
	<div class="style1_titre_deco"><h3><font style="text-transform: none;"><spring:message code="foreignStd.listForeignStdTitle1"/> <c:out value="${university.name}" /> <spring:message code="foreignStd.listForeignStdTitle2ESPEME"/>
	</font></h3>
	</div>
<!--
<c:if test="${ not empty otherSchool}">
					<div class="ligne">
						<input id="otherSchool" name="${otherSchool}" class="input_submit" size="50" value="Go <spring:message code="foreignStd.listForeignStdTitle2"/>" />
					</div>
		 			<br/>
</c:if>
-->
	<br/>This online nomination system will be open from April 12th to <strong>April 30th, 2022</strong> for nominations regarding the <strong>Fall Semester and Full year</strong> exchange.<br/><br/>According to the exchange agreement between <c:out value="${university.name}" /> and EDHEC BBA you can nominate the following number of students for the academic year 2022/2023 :
<!--
		...
		<c:when test='${not empty foreignStudentList}'>
			<c:forEach  items='${foreignStudentList}' var='student'>
				<tr class="odd" >
					...
					<c:if test="${forceadmin == 'Y'}">
					<td>
							<a class="registerAccount" rel="#registered" href="registerforeignStudent?foreignStudentId=<c:out value="${student.id}" />&school=BBA"><img title="Register a student" src="<c:url value="/images/unlock.png"/>" /></a>&nbsp;&nbsp;
							<a class="confirmAccount" rel="#confirmed" href="confirmforeignStudent?foreignStudentId=<c:out value="${student.id}" />&school=BBA"><img title="Confirm a student" src="<c:url value="/images/lock.png"/>" /></a>&nbsp;&nbsp;
						    <a class="cancelAccount" rel="#canceled" href="cancelStudent?foreignStudentId=<c:out value="${student.id}" />&school=BBA"><img title="Cancel a student" src="<c:url value="/images/cancel.png"/>" /></a>
